...
## 2026-09-18 - [Persistent Dish Favorites Toggle with Screen Reader Live Announcements and ARIA Pressed State Syncing]
**Learning:** When users browse restaurant catalog items, having a lightweight way to favorite or bookmark dishes (`♡`/`♥`) directly on item cards greatly improves delight and return usability. Using HTML `<button type="button" class="dish-favorite-btn">` with dynamic `aria-pressed="true|false"`, synchronized `aria-label` / `title` attributes, screen reader live region announcements (`announceCartAction`), and `localStorage` persistence creates a fully accessible, stateful micro-interaction pattern without adding external dependencies.
**Action:** Pair interactive favorite toggle buttons with `aria-pressed`, live announcements, and `localStorage` persistence, ensuring focus rings and hover states match the active design tokens in both light and dark themes.
