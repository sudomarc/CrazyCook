...
## 2026-09-28 - [WAI-ARIA Menu Category Filter Tabs with Keyboard Cycling and Live Announcements]
**Learning:** In single-page menu/catalog layouts with CSS Grid category blocks, adding category filter tabs (`role="tablist"`, `role="tab"`) with `aria-selected` toggling and native HTML `hidden` property enforcement (`[hidden] { display: none !important; }`) prevents CSS grid display properties from overriding element hiding. Pairing this with WAI-ARIA arrow key navigation (`ArrowLeft`, `ArrowRight`, `Home`, `End`) and screen-reader live region announcements (`announceCartAction`) provides keyboard users and screen readers with instant feedback on filtered item counts.
**Action:** Implement category filter tab controls using standard `role="tablist"` / `role="tab"` ARIA markup, cycle focus using arrow keys, and enforce explicit `[hidden] { display: none !important; }` rules in CSS.
